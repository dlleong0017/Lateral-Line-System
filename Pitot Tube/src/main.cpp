...
#define SENSOR_OFFSET_KPA -0.15f
...
float voltageToPressure(float pin_voltage)
{
  // Reconstruct voltage before the voltage divider.
  float sensor_voltage = pin_voltage * (5.0f / 3.3f);

  float raw_pressure_kpa =
      (sensor_voltage / 5.0f - 0.5f) / 0.2f;

  return raw_pressure_kpa + SENSOR_OFFSET_KPA;
}


float pressureToVelocity(float pressure_kpa)
{
...
void setup()
{
  ...
  Serial.println(
      "t_ms,velocity_m_s,pressure_kpa,pin_voltage_v"
  );
}
...
void loop()
{
  ...
  // Take one ADC measurement and derive everything from it.
  float pin_voltage = readVoltage();

  float pressure_kpa =
      voltageToPressure(pin_voltage);

  float fluid_velocity =
      pressureToVelocity(pressure_kpa);
